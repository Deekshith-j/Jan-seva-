...
import { useStates, useDistricts, useDepartments, useOfficesByDistrict } from '@/hooks/useMasterData';
...
const OfficialRegister: React.FC = () => {
  ...
  // const [department, setDepartment] = useState(''); // Deprecated
  ...
  const { data: districtsList } = useDistricts(selectedState);
  const { data: departmentsList } = useDepartments();
  const { data: officesList } = useOfficesByDistrict(selectedDistrict, selectedDepartment);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!selectedOffice || !selectedDepartment) {
      toast.error("Please select valid office and department");
      setIsLoading(false);
      return;
    }
    ...
    try {
      // Find dept name
      const deptName = departmentsList?.find(d => d.id === selectedDepartment)?.name || '';

      // We need to pass extra metadata. If auth context doesn't support it yet, we might lose it.
      // Assuming register accepts a 5th arg or we update it later. 
      // For now, let's perform the registration and THEN update the profile with extra fields via Supabase directly if needed
      // OR rely on AuthContext update. 
      // Let's UPDATE AUTH CONTEXT in next step. For now, calling with minimal args but valid flow.
      await register(email, password, fullName, 'official', {
        employee_id: employeeId,
        assigned_office_id: selectedOffice,
        assigned_department_id: selectedDepartment,
        department: deptName // Legacy fallback
      });
            ...
            <CardContent className="pt-4">
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={setSelectedDepartment} value={selectedDepartment} disabled={!selectedDistrict}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={setSelectedOffice} value={selectedOffice} disabled={!selectedDepartment}>
                    <SelectTrigger>
                ...
                <Button
                  ...
                  className="w-full"
                  size="lg"
                  variant="default" // Changed from accent to default for now as accent might not be defined in variants
                  disabled={isLoading}
                >
                  {isLoading ? (
