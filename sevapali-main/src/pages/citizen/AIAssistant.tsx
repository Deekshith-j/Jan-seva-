...
import { Bot, Send, Mic, MicOff, User, Sparkles } from 'lucide-react';
...
const AIAssistant: React.FC = () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          await handleVoiceQuery(base64Audio);
        };
      ...
      mediaRecorder.start();
      setIsListening(true);
      toast.info(language === 'mr' ? 'ऐकत आहे...' : 'Listening...');
    } catch (err) {
      console.error('Error accessing microphone:', err);
      toast.error(language === 'mr' ? 'मायक्रोफोन ऍक्सेस नाकारला' : 'Microphone access denied');
    }
  };
  ...
  const handleVoiceQuery = async (audioBase64: string) => {
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
          role: 'user',
          content: query, // Transcribed text
          timestamp: new Date()
        }
      ]);
      ...
      if (language === 'mr') utterance.lang = 'mr-IN'; // Marathi if available, fallback to default (usually Hindi or English)
      window.speechSynthesis.speak(utterance);

    } catch (err) {
      console.error('Voice Query Error:', err);
      toast.error(language === 'mr' ? 'समजले नाही, पुन्हा प्रयत्न करा' : 'Could not understand, please try again');
    } finally {
      setIsLoading(false);
    }
  };
